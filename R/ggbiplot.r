                      ...
                      var.quantile.threshold = 0, ...)
{
 ...
 if(inherits(pcobj, 'prcomp')){
  ...
  if(var.axes) {
    ...
    # Draw directions
    g <- g +
      geom_segment(data = df.v[quantile(sqrt(df.v$xvar ^ 2 + df.v$yvar ^ 2), var.quantile.threshold) <= sqrt(df.v$xvar ^ 2 + df.v$yvar ^ 2),],
  ...
  if(var.axes) {
    g <- g + 
    geom_text(data = df.v[quantile(sqrt(df.v$xvar ^ 2 + df.v$yvar ^ 2), var.quantile.threshold) <= sqrt(df.v$xvar ^ 2 + df.v$yvar ^ 2),],
